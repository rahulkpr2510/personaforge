  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <p className="text-[8px] text-muted-foreground leading-none mt-0.5">conf.</p>
      </div>
    </div>
  );
}
...
function Section({
  ...
  return (
    <div className={cn("space-y-1.5", accent && "rounded-lg border border-border bg-muted/20 p-3")}>
...
export function EvidenceInspectorDrawer({
  ...
  const levelMeta = f?.evidenceLevel ? LEVEL_META[f.evidenceLevel] : LEVEL_META["INFERRED"];
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          ...
          {severityMeta && f?.severity && (
            <div className={cn("rounded-lg border px-3 py-2", severityMeta.bg, severityMeta.border)}>
            ...
            <Section icon={Shield} label="Confidence basis">
              ...
              {f.evidenceLevel && (
                <p className="mt-1 text-xs text-muted-foreground">
                  Evidence level: <span className={cn("font-semibold", levelMeta.color)}>{levelMeta.label}</span>
                  {" "}— {levelMeta.desc}
                </p>
              )}
            </Section>
          ...
          {/* Affected pages */}
          {f?.affectedPages && f.affectedPages.length > 0 && (
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                <AlertTriangle className="h-3.5 w-3.5 text-(--pf-accent)" />
                <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">
                  Affected pages
                </p>
              </div>
              <div className="space-y-1">
                {f.affectedPages.slice(0, 5).map((url, i) => (
                  <a
                    key={i}
                    href={url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1.5 rounded-lg border border-border bg-muted/30 px-3 py-2 text-xs text-foreground hover:bg-muted/50 transition-colors group"
                  >
